...
export function EditProductModal({ 
  ...
  useEffect(() => {
    if (isOpen && product) {
      ...
      api.get('/categories').then(res => {
        ...
        if (res.data.length > 0 && !product.categoryId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error('Erro ao editar produto. Verifique os campos ou códigos duplicados.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-end bg-black/60 backdrop-blur-sm transition-all">
      <div className="bg-zinc-950 border-l border-zinc-800 w-full max-w-md h-full shadow-2xl animate-in slide-in-from-right duration-300 flex flex-col">
        ...
        <div className="flex justify-between items-center p-6 border-b border-zinc-800 bg-zinc-900/50">
          <h2 className="text-xl font-bold text-white flex items-center gap-3"><Edit3 className="text-blue-500" /> Editar Produto</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 flex-1 overflow-y-auto space-y-5 custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-semibold text-zinc-400 mb-1.5 block">Alterar Estoque *</label>
              <input required type="number" value={formData.stock} onChange={e => setFormData({...formData, stock: e.target.value})} className="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 text-blue-400 font-black focus:outline-none focus:border-blue-500" />
            </div>
            <div>
