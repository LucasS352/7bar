...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  addItem: (product, quantity = 1) => {
    ...
    let newItems;
    if (existing) {
      newItems = items.map(i => i.id === product.id ? { ...i, quantity: i.quantity + quantity, subtotal: (i.quantity + quantity) * Number(i.priceSell) } : i);
    } else {
      newItems = [...items, { ...product, quantity, subtotal: quantity * Number(product.priceSell) }];
    }
  ...
  updateQuantity: (id, quantity) => {
    ...
    const newItems = get().items.map(i => i.id === id ? { ...i, quantity, subtotal: quantity * Number(i.priceSell) } : i);
    set({ items: newItems, total: newItems.reduce((acc, i) => acc + i.subtotal, 0) });
  },
